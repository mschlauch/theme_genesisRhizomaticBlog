...
function genesis_RhizomaticTourism_preprocess_page(&$vars) {
	global $theme;
	 ...
	 // Set variables for the logo and site_name.
	  if ($vars['logo']) {
	    $vars['site_logo'] = '<a href="' . $vars['front_page'] . '" title="' . t('Home page') . '" rel="home"> <img src="' . $vars['logo'] . '" /> </a>';
	  }
	  else {
	    $vars['site_logo'] = '';
	  }
...
function genesis_RhizomaticTourism_preprocess_node(&$vars, $hook) {
//submitted line onder artikelen
$vars['submitted'] = t('by !username - @datetime', array(
'!username' => $vars['name'],
'@datetime' => date("j F Y", $vars['created']),
/**'!reacties' => l(format_plural($vars['comment'], '1 reactie', '@count reacties'), drupal_lookup_path(alias,$vars['uri']['path']).'#comments') */
));
